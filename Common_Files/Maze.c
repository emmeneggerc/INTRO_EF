...
TURN_Kind MAZE_SelectTurn(REF_LineKind prev, REF_LineKind curr, bool rule) {
	...
	if (prev == REF_LINE_NONE && curr == REF_LINE_NONE) { /* dead end */
		...
		return TURN_LEFT90;
	} else if (prev == REF_LINE_RIGHT && curr == REF_LINE_NONE) { // rechte Ecke
		MAZE_AddPath(TURN_RIGHT90);
		return TURN_RIGHT90;
	} else if (prev == REF_LINE_FULL && curr == REF_LINE_NONE) {	// T-Stück
		if (rule) {
			MAZE_AddPath(TURN_LEFT90);
	...
	} else if (prev == REF_LINE_LEFT && curr == REF_LINE_STRAIGHT) {// linke Abzweigung
		if (rule) {
			MAZE_AddPath(TURN_LEFT90);
			return TURN_LEFT90;
		} else {
	...
	} else if (prev == REF_LINE_RIGHT && curr == REF_LINE_STRAIGHT) {// rechte Abzweigung
		if (rule) {
			MAZE_AddPath(TURN_STRAIGHT);
			return TURN_STRAIGHT;
		} else {
